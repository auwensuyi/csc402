...
class HW1 {
	...
	public static int distanceBetweenMinAndMax(double[] list) {
		...
		distance = Math.abs(indexMax - indexMin);
	...
	public static double[] removeDuplicates(double[] list) {
		int newArraySize = numUnique(list);
		double[] newArray = null;
		if(newArraySize == 0){
		return list;
		}else{
		newArray = new double[newArraySize];
		int size = list.length;
		double unique = list[0] - 1;
		int innerCount = 0;
		for(int i = 0; i < size; i++){
...
*/
// Everything works based on sample output. 